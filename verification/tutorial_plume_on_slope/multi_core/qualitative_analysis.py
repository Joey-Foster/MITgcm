...
parser.add_argument('-swf', '--smooth+wallfuncs', help='file path to state_global.nc with smooth ' 
                    'bathymetry and which uses wall functions', dest='swf')
...
parser.add_argument('-wwf', '--wiggly+wallfuncs', help='file path to state_global.nc with '
                    'multiscale bathymetry and which uses wall functions', dest='wwf')
...
vmin = thetas['smooth'].isel(T=-1).min().values
vmax = thetas['smooth'].isel(T=-1).max().values
...
if (args.swf is not None) and (args.wwf is not None):
    ...
    thetas['swf'].isel(T=-1).plot(ax=ax[1,0], **plotting_customisation)
    ax[1,0].set_xlabel('X [m]')
    ax[1,0].set_ylabel('Depth [m]')
    ax[1,0].set_title('')
    
    thetas['wwf'].isel(T=-1).plot(ax=ax[1,1], **plotting_customisation)
    ...
    im = thetas['smooth'].isel(T=-1).plot(ax=ax[0], **plotting_customisation)
    ax[0].set_xlabel('')
    ax[0].set_ylabel('Depth [m]')
    ax[0].set_title('Smooth bathymetry')
    
    thetas['wiggly'].isel(T=-1).plot(ax=ax[1], **plotting_customisation)
    ax[1].set_xlabel('')
    ax[1].set_ylabel('')
    ax[1].set_title('Multiscale bathymetry')
