...
C     and roughness length
     ...
     &     bottomDragCoeffW, bottomDragCoeffS
      Real*8 bottomDragCoeffW(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 bottomDragCoeffS(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     and roughness length, assuming von Karman constant = 0.4
      DO bj=myByLo(myThid), myByHi(myThid)
       DO bi=myBxLo(myThid), myBxHi(myThid)
        DO j=1-OLy,sNy+OLy
         ...
         ENDDO
        ENDDO
        IF ( zRoughBot .GT. 0.D0 .AND. usingZCoords ) THEN
         recip_zRoughBot = 1.D0 / zRoughBot
         DO j=1-OLy,sNy+OLy
     ...
     &           0.5*drF(kLow)*hFacW(i,j,kLow,bi,bj)*recip_zRoughBot )
           ...
           ELSE
            bottomDragCoeffW(i,j,bi,bj) = 0.D0
           ENDIF
          ENDDO
         ENDDO
     ...
     &           0.5*drF(kLow)*hFacS(i,j,kLow,bi,bj)*recip_zRoughBot )
           ...
           ELSE
            bottomDragCoeffS(i,j,bi,bj) = 0.D0
           ENDIF
          ENDDO
         ENDDO
        ELSEIF ( zRoughBot .GT. 0.D0 ) THEN
